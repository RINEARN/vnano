		...
		switch (opcode) {
			...
			*/
			...
			case ELEM : {
				...
				// ELEMは可変長オペランドで dest, src, index0, index1, ... と並ぶので、index 部のみを詰めた DataContainer 配列を用意
				...
				//   > そもそも ELEM で index... 部を、配列データを格納する単引数で渡すのは?
				//     > そうすると ELEM を呼ぶ前に全次元のインデックス(スカラ)値を配列にパックする命令が必要になって、結局遅いと思う
