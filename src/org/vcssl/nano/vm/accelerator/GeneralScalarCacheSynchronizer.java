...
public class GeneralScalarCacheSynchronizer extends CacheSynchronizer {
	...
	public final void synchronizeFromCacheToMemory() {
		...
		for (int i=0; i<n; i++) {
			...
			if (caches[i] instanceof Int64ScalarCache) {
				((long[])(containers[i].getArrayData()))[ containers[i].getArrayOffset() ] = ((Int64ScalarCache)caches[i]).value;

			} else if (caches[i] instanceof Float64ScalarCache) {
				((double[])(containers[i].getArrayData()))[ containers[i].getArrayOffset() ] = ((Float64ScalarCache)caches[i]).value;

			} else if (caches[i] instanceof BoolScalarCache) {
				((boolean[])(containers[i].getArrayData()))[ containers[i].getArrayOffset() ] = ((BoolScalarCache)caches[i]).value;
	...
	public final void synchronizeFromMemoryToCache() {
		...
		for (int i=0; i<n; i++) {
			...
			// 将来的に containers[i].getDataType() に変更？
			if (caches[i] instanceof Int64ScalarCache) {
				((Int64ScalarCache)caches[i]).value = ((long[])(containers[i].getArrayData()))[ containers[i].getArrayOffset() ];

			} else if (caches[i] instanceof Float64ScalarCache) {
				((Float64ScalarCache)caches[i]).value = ((double[])(containers[i].getArrayData()))[ containers[i].getArrayOffset() ];

			} else if (caches[i] instanceof BoolScalarCache) {
				((BoolScalarCache)caches[i]).value = ((boolean[])(containers[i].getArrayData()))[ containers[i].getArrayOffset() ];
