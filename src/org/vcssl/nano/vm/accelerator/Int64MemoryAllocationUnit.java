...
public class Int64MemoryAllocationUnit extends AcceleratorExecutionUnit {
	...
	public AcceleratorExecutionNode generateNode(
		...
		if (instruction.getOperationCode() == OperationCode.ALLOC) {
			...
			if (arrayRank == DataContainer.ARRAY_RANK_OF_SCALAR) {
				return new Int64ScalarAllocNode(operandContainers[0], nextNode);
			} else {
				return new Int64VectorAllocNode(
	...
	private final class Int64ScalarAllocNode extends AcceleratorExecutionNode {
		protected final DataContainer<?> container0;

		public Int64ScalarAllocNode(DataContainer<?> container0, AcceleratorExecutionNode nextNode) {
		...
		public final AcceleratorExecutionNode execute() {
			...
			if (currentData instanceof long[] && ((long[])currentData).length != 0) {
				// ※ offset 値が 0 でない対象に対して ALLOC される事は、現状の言語仕様では発生しない
				return this.nextNode;
			}

			// メモリ確保が必要なので、確保してデータコンテナに持たせる
			((DataContainer<long[]>)this.container0).setArrayData(
				new long[ DataContainer.ARRAY_SIZE_OF_SCALAR ],    // data
				0,                                                 // offset
	...
	private final class Int64VectorAllocNode extends AcceleratorExecutionNode {
		...
		public final AcceleratorExecutionNode execute() {
			...
			if (currentData instanceof long[] && ((long[])currentData).length == requiredSize) {
				// ※ offset 値が 0 でない対象に対して ALLOC される事は、現状の言語仕様では発生しない
				return this.nextNode;
			}
	...
	private final class Int64AllocrNode extends AcceleratorExecutionNode {
		protected final DataContainer<?> targetContainer;
		protected final DataContainer<?> sameLengthContainer;

		public Int64AllocrNode(
